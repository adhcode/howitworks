...
const TestimonialCard = ({ rating, title, content, author }: TestimonialCardProps) => {
    return (
        <div className="bg-white rounded-[10px] p-6 border border-[#E5E7EB] hover:shadow-lg transition-all">
            {/* Rating Stars */}
            <div className="flex gap-1 mb-4">
                {[...Array(5)].map((_, index) => (
                    <div key={index} className="w-8 h-8 rounded-full border border-[#E5E7EB] flex items-center justify-center">
                        <svg
                            className={`w-5 h-5 ${index < rating ? 'text-[#FFB300]' : 'text-[#E5E7EB]'}`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
                        >
            ...
            <p className="text-[#666666] text-[16px] mb-6 leading-relaxed">{content}</p>
