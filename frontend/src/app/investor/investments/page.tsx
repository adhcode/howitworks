...
export default function InvestorInvestments() {
  ...
  const { data: investmentsData, isLoading: loading, error } = useInvestments({
    page: currentPage,
    limit: 10,
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Pagination */}
      {investmentsData && investmentsData.totalPages > 1 && (
        <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6 rounded-lg shadow">
          <div className="flex-1 flex justify-between sm:hidden">
            ...
            <button
              onClick={() => setCurrentPage(Math.min(investmentsData.totalPages, currentPage + 1))}
              disabled={currentPage === investmentsData.totalPages}
            ...
            <div>
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">
                  {(currentPage - 1) * investmentsData.limit + 1}
                </span>{' '}
                to{' '}
                <span className="font-medium">
                  {Math.min(currentPage * investmentsData.limit, investmentsData.total)}
                </span>{' '}
                of{' '}
                <span className="font-medium">{investmentsData.total}</span> results
              </p>
            </div>
            <div>
